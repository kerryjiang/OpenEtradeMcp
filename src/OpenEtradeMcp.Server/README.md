...
```json
{
  "mcpServers": {
    "etrade": {
      "command": "dotnet",
      "args": ["run", "--project", "/path/to/OpenEtradeMcp.Server"],
      "env": {
...
Ensure the `etrade-api.yaml` file is in the output directory. Check that the OpenEtradeMcp project copies it to the output.
